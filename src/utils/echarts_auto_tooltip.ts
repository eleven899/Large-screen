...
export const autoToolTip = (chart, chartOption, options) => {
  ...
  let dataIndex: any = 0; // 数据索引，初始化为-1，是为了判断是否是第一次执行
  let seriesIndex: any = 0; // 系列索引
  let timeTicket: any = 0;
  const seriesLen: any = chartOption.series.length; // 系列个数
  let dataLen: any = 0; // 某个系列数据个数
  ...
  function autoShowTip() {
    function showTip() {
      ...
      const series: any = chartOption.series;
